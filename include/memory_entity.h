...
namespace bear
{
	...
	_T & cast_memory(array_ptr<char> mem)
	{
		if (sizeof(_T) != mem.size())
			throw bear_exception(exception_type::pointer_outof_range,
	...
	public:
		...
		bool validate() const
		{
			...
			if ((data_start & 3) || data_start < 4 || data_start > _ptr.size())return false;

			size_type sz = data_start / sizeof(size_type) - 1;

			for (size_type i = 0; i < sz; ++i)
			{
				size_type data_end = _get_size(i + 1);
			...
			if (data_start != _ptr.size())return false;
		...
		{
			return _get_size(0) / sizeof(size_type) - 1;
		}
	...
	public:
		const_entity_list(array_ptr<char> mem) : base(mem) {}
